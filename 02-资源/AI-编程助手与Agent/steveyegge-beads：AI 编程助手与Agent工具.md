# steveyegge-beads：AI 编程助手与Agent工具

## 文档信息

- 发布日期：2026-02-22
- 来源：https://github.com/steveyegge/beads
- 发现评分：92
- Stars：16945

## 摘要

**一句话总结**
Beads 是一个高关注度的开源项目，专为 AI 编程助手（Coding Agent）提供记忆升级（Memory upgrade）功能。

**关键点**
* **项目名称**：Beads
* **核心定位**：AI 编程助手与 Agent 工具。
* **主要功能**：作为专为 AI 编程助手（Coding Agent）设计的记忆升级（Memory upgrade）工具。
* **社区热度**：在 GitHub 上获得了 16,945 个 Stars，显示出极高的社区关注度。
* **项目地址**：代码托管于 GitHub（https://github.com/steveyegge/beads）。

## 功能与定位
Beads 是一个专为 AI 编程助手（Coding Agent）设计的记忆升级（Memory upgrade）工具项目。

## 链接
- GitHub 仓库：https://github.com/steveyegge/beads


## 关联主题

- [[00-元语/memory]]
- [[00-元语/AI]]
- [[00-元语/Agent]]
- [[00-元语/github]]
