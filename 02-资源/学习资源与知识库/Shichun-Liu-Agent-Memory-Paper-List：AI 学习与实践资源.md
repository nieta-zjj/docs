---
title: "Shichun-Liu-Agent-Memory-Paper-List：AI 学习与实践资源"

发布日期: "2026-02-22"
来源: "https://github.com/Shichun-Liu/Agent-Memory-Paper-List"
发现评分: "92"
Stars: "1258"
---

## 摘要

**一句话总结**
该项目是一个托管在 GitHub 上的学术资源库，专门收录并整理了综述论文《Memory in the Age of AI Agents: A Survey》中涉及的 AI Agent 记忆相关论文清单。

**核心要点**
* **项目名称**：Shichun-Liu/Agent-Memory-Paper-List。
* **项目定位**：专注于 AI Agent 领域的学术资源库。
* **核心功能**：提供与 AI Agent 记忆（Memory）主题相关的学术论文详细清单。
* **内容来源**：主要基于并整理自综述论文《Memory in the Age of AI Agents: A Survey》。
* **访问渠道**：项目开源托管于 GitHub 平台（https://github.com/Shichun-Liu/Agent-Memory-Paper-List）。

## 功能与定位

该项目是一个学术资源库，主要收录并整理了综述论文《Memory in the Age of AI Agents: A Survey》（AI Agent 时代的记忆：综述）中所涉及的论文列表。

## 核心功能

* 提供与 AI Agent 记忆（Memory）主题相关的学术论文清单。

## 链接

* GitHub 仓库：https://github.com/Shichun-Liu/Agent-Memory-Paper-List

## 关联主题

- [[00-元语/memory]]
- [[00-元语/AI]]
- [[00-元语/Agent]]
- [[00-元语/github]]
- [[00-元语/learning-resource]]
- [[00-元语/paper]]
